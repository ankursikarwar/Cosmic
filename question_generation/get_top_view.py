...
def place_cam_over_exterior(cam, margin=1.08):
    ...
    bbox = combined_bbox(exts)
    if bbox is None:
        print("[WARN] No visible 'exterior' objects; falling back to scene bbox.")
        bbox = scene_bbox()
    if bbox is None:
        cam.data.type = 'ORTHO'
        cam.location = (0, 0, 5)
    ...
    sc_mn, sc_mx = (scene_bbox() or (mn, mx))
    scene_h = max(0.1, sc_mx.z - sc_mn.z)
    cam.location.z = sc_mx.z + scene_h + 0.5
