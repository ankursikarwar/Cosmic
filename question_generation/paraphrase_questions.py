...
sys.path.append("/home/mila/a/ankur.sikarwar/Work/MultiAgent/")
import json
import argparse
from typing import Dict, Any, List, Tuple
from pathlib import Path # Import Path for type hinting
from question_generation_v2.llm_utils import create_vllm_client, create_openai_client
...
# • Convert names like:
#    "Simple Desk (White, next to a white shelf)" → "white desk located next to a white shelf".
#    "Wall Art (small)" → "small wall art".
#    "Monitor" → "Computer Monitor" (Always paraphrase a monitor as a computer monitor, and do not do this for any other objects)
# • Neighboring relations can be 'next', 'near', or 'on'. When the relation is 'on', you are to use common sense to figure out which is the object on which the other object is kept
#    "Simple Desk (Black, on a green lamp)" → "black desk on which a green lamp is kept" (since lamp can be kept on desk but not the opposite)
#    "Lamp (beige, on a black Desk)" → "beige lamp on a black desk"
#    "Shelf (Black, on a TV)" → "black shelf on which a TV is kept" (since shelf cannot be kept on the TV but the other way round is more realistic)
# • When parphrasing objects like lamp, sizes are not 'big' or 'small' but are 'floor' or 'desk'. So convert below:
#    "Lamp (beige, Desk, on a black Shelf)" → "beige desk lamp kept on a black shelf"
#    "Lamp (beige, Desk, on a white Desk)" → "beige desk lamp kept on a white desk"
#    "Sofa (yellow, near a shutter window)" → "yellow sofa near a window with shutters"
...
#    "Bed (beige, near a white Shelf)" → "bed with beige bedframe near a white shelf"
#    "Sofa (yellow, near a green bed)" → "yellow sofa near a bed with green bedframe"
# • When paraphrasing windows, the color is either "shutter" or an actual "color" which refers to curtain. So convert below:
#    "Window (shutter, near a white Shelf)" → "window with shutter near a white shelf"
#    "Window (purple) → "window with the purple curtains"
# •⁠ IMPORTANT: Do not eliminate any descriptors from any object. For example, if the object is "Door (white, big, near brown desk)", ensure that the paraphrase includes all the three descriptors always. "Door (white, big, near brown desk)" → "big, white door near a brown desk". Also when referring to neighbouring objects, use "a" and not "the", as it is not guaranteed to be unique.
...
"""
    ...
    if category == "counting_questions":
...
"""
    elif category == "spatial_orientation_questions":
        json_to_paraphrase["question"] = question_obj["question"]
        json_to_paraphrase["question_both_views"] = question_obj["question_both_views"]
        prompt = base_instruction + f"""
...
Here is the input JSON to paraphrase:
"""
    elif category == "perspective_taking_questions":
        json_to_paraphrase["question"] = question_obj["question"]
        json_to_paraphrase["question_both_views"] = question_obj["question_both_views"]
        prompt = base_instruction + f"""
Paraphrase the 'question' and 'question_both_views' fields.
...
- "Relative to your partner's perspective,"
- "With respect to your partner's perspective,"
- any other similar phrase as above

When the main object includes descriptors like "near", "next to", or "on", ensure:
- The relationship between objects remains clear and grammatical.
- Use "that is" if needed for clarity (e.g., "the green side table that is next to a lamp").
- Avoid awkward phrasing or descriptor clashes.
...
Examples:
Original: "Which direction is the white shelf located near a sofa?"
→ "From your partner's perspective, which direction is the white shelf that is near a sofa?"
→ "Relative to your partner's perspective, in which direction is the white shelf that stands near a sofa?"
...
def paraphrase_across_scenes(
    ...
    for scene in tqdm(scenes, desc="Scenes"):
        ...
        # MAP CHANGE
        # map_input_path = scene / "map_questions.json"
        ...
        if not input_path.exists():
            ...
            continue
        # MAP CHANGE
        # if not map_input_path.exists():
        #     print(f"⚠️ {scene.name}: {map_input_path.name} not found, skipping.")
        #     continue
        ...
        #     map_data = json.load(f) # map_data is a list of questions
        # data["map_questions"] = map_data
        ...
        for category, questions in data.items():
            ...
            for idx, q in enumerate(questions):
                ...
                try:
                    ...
                    if category in ["anchor_questions", "relative_distance_questions",
                                   "spatial_orientation_questions", "perspective_taking_questions",
                                   "map_questions"]:
                        if "options" not in q:
                ...
                # Increased token limit for potentially large map_questions
                max_tokens = 2048
    ...
    for out, (scene, category, idx, original_q) in zip(all_outputs, query_to_scene_map):
        ...
        categories_with_options = ["anchor_questions", "relative_distance_questions",
                                   "spatial_orientation_questions", "perspective_taking_questions",
                                   "map_questions"]
